...
import { ExternalProvidersRepository } from '@modules/external-providers/external-providers.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('ExternalProvidersModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<ExternalProvidersRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test ExternalProviders',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [ExternalProvidersModule],
    })
      .overrideProvider(ExternalProvidersRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<ExternalProvidersRepository>(ExternalProvidersRepository) as any;
  });
  ...
  describe('Flujos HTTP - ExternalProviders', () => {
    it('POST /external-providers — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test', providerType: 'UNIVERSITY', annualAllocationId: 'some-id', providedJourneyTime: 10 };
    ...
    it('GET /external-providers — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/external-providers?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /external-providers/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/external-providers/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /external-providers/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/external-providers/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /external-providers/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/external-providers/test-id-1');
