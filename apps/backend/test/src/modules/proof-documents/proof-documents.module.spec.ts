...
import { ProofDocumentsModule } from '@modules/proof-documents/proof-documents.module';
import { ProofDocumentsRepository } from '@modules/proof-documents/proof-documents.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('ProofDocumentsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<ProofDocumentsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test ProofDocuments',
    status: 'ACTIVE',
  };
  ...
  const mockRepository = {
    ...
    deleteById: jest.fn(),
    delete: jest.fn(),
    generateNextCode: jest.fn().mockResolvedValue("CODE-001"),
    updateGoogleDriveIdAndName: jest.fn(),
    count: jest.fn(),
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [ProofDocumentsModule],
    })
      .overrideProvider(ProofDocumentsRepository).useValue(mockRepository)
      .overrideProvider(DtoValidator).useValue(mockDtoValidator)
      .overrideProvider(PrismaService).useValue({ question: { count: jest.fn().mockResolvedValue(0) }, sinaesDocumentHistory: { create: jest.fn() } })
    ...
    repository = moduleFixture.get<ProofDocumentsRepository>(ProofDocumentsRepository) as any;
  });
  ...
  describe('Flujos HTTP - ProofDocuments', () => {
    it('POST /proof-documents — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test ProofDocuments' };
    ...
    it('GET /proof-documents — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/proof-documents?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /proof-documents/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/proof-documents/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /proof-documents/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/proof-documents/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /proof-documents/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/proof-documents/test-id-1');
