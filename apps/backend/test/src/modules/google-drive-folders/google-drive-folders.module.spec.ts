...
import { GoogleDriveFoldersModule } from '@modules/google-drive-folders/google-drive-folders.module';
import { GoogleDriveFoldersRepository } from '@modules/google-drive-folders/google-drive-folders.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('GoogleDriveFoldersModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<GoogleDriveFoldersRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test GoogleDriveFolders',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [GoogleDriveFoldersModule],
    })
      .overrideProvider(GoogleDriveFoldersRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<GoogleDriveFoldersRepository>(GoogleDriveFoldersRepository) as any;
  });
  ...
  describe('Flujos HTTP - GoogleDriveFolders', () => {
    it('POST /google-drive-folders — Crea registro — responde 201', async () => {
      const createDto = { 
        name: 'Test GoogleDriveFolders', 
        courseId: 'asd', 
        academicCycleId: 'asd', 
    ...
    it('GET /google-drive-folders — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/google-drive-folders?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /google-drive-folders/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/google-drive-folders/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /google-drive-folders/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/google-drive-folders/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /google-drive-folders/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/google-drive-folders/test-id-1');
