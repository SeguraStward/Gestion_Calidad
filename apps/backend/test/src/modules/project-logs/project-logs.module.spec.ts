...
import { ProjectLogsRepository } from '@modules/project-logs/project-logs.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('ProjectLogsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<ProjectLogsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test ProjectLogs',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [ProjectLogsModule],
    })
      .overrideProvider(ProjectLogsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<ProjectLogsRepository>(ProjectLogsRepository) as any;
  });
  ...
  describe('Flujos HTTP - ProjectLogs', () => {
    it('POST /project-logs — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test ProjectLogs' };
