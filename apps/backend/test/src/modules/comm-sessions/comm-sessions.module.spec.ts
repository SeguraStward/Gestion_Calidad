...
import { CommSessionsModule } from '@modules/comm-sessions/comm-sessions.module';
import { CommSessionsRepository } from '@modules/comm-sessions/comm-sessions.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('CommSessionsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<CommSessionsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test CommSessions',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [CommSessionsModule],
    })
      .overrideProvider(CommSessionsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<CommSessionsRepository>(CommSessionsRepository) as any;
  });
  ...
  describe('Flujos HTTP - CommSessions', () => {
    it('POST /comm-sessions — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test CommSessions' };
