...
import { AcademicLoadGroupsModule } from '@modules/academic-load-groups/academic-load-groups.module';
import { AcademicLoadGroupsRepository } from '@modules/academic-load-groups/academic-load-groups.repository';

describe('AcademicLoadGroupsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<AcademicLoadGroupsRepository>;

  const mockEntity = {
    id: 'group-1',
    number: 'Grupos1', status: 'ACTIVE',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [AcademicLoadGroupsModule],
    })
      .overrideProvider(AcademicLoadGroupsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<AcademicLoadGroupsRepository>(AcademicLoadGroupsRepository) as any;
  });
  ...
  describe('Flujos HTTP - AcademicLoadGroups', () => {
    it('POST /academic-load-groups — Crea registro — responde 201', async () => {
      const createDto = { number: 'Grupos1', status: 'ACTIVE' };
      repository.save.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .post('/academic-load-groups')
        .send(createDto);
    ...
    it('GET /academic-load-groups — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/academic-load-groups?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /academic-load-groups/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/academic-load-groups/group-1')
        .expect(200);

      expect(response.body.id).toBe('group-1');
    });

    it('PUT /academic-load-groups/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/academic-load-groups/group-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /academic-load-groups/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/academic-load-groups/group-1');
