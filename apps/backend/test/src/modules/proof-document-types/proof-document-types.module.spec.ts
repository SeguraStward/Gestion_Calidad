...
import { ProofDocumentTypesModule } from '@modules/proof-document-types/proof-document-types.module';
import { ProofDocumentTypesRepository } from '@modules/proof-document-types/proof-document-types.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('ProofDocumentTypesModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<ProofDocumentTypesRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test ProofDocumentTypes',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [ProofDocumentTypesModule],
    })
      .overrideProvider(ProofDocumentTypesRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<ProofDocumentTypesRepository>(ProofDocumentTypesRepository) as any;
  });
  ...
  describe('Flujos HTTP - ProofDocumentTypes', () => {
    it('POST /proof-document-types — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test ProofDocumentTypes' };
    ...
    it('GET /proof-document-types — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/proof-document-types?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /proof-document-types/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/proof-document-types/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /proof-document-types/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/proof-document-types/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /proof-document-types/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/proof-document-types/test-id-1');
