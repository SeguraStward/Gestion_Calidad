...
import { IntellectualProductionsRepository } from '@modules/intellectual-productions/intellectual-productions.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('IntellectualProductionsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<IntellectualProductionsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test IntellectualProductions',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [IntellectualProductionsModule],
    })
      .overrideProvider(IntellectualProductionsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<IntellectualProductionsRepository>(IntellectualProductionsRepository) as any;
  });
  ...
  describe('Flujos HTTP - IntellectualProductions', () => {
    it('POST /intellectual-productions — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test IntellectualProductions' };
    ...
    it('GET /intellectual-productions — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/intellectual-productions?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /intellectual-productions/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/intellectual-productions/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /intellectual-productions/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/intellectual-productions/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /intellectual-productions/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/intellectual-productions/test-id-1');
