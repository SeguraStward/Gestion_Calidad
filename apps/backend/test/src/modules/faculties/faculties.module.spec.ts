...
describe('FacultiesModule (Modular)', () => {
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(FacultiesRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<FacultiesRepository>(FacultiesRepository) as any;
  });
  ...
  describe('Flujos HTTP - Faculties', () => {
    it('POST /faculties — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test Faculties' };
    ...
    it('GET /faculties — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/faculties?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /faculties/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/faculties/faculty-1')
        .expect(200);

      expect(response.body.id).toBe('faculty-1');
    });

    it('PUT /faculties/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/faculties/faculty-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /faculties/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/faculties/faculty-1');
