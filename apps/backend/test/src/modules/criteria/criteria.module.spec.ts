...
describe('CriteriaModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<CriteriaRepository>;
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(CriteriaRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<CriteriaRepository>(CriteriaRepository) as any;
  });
  ...
  describe('Flujos HTTP - Criteria', () => {
    it('POST /criteria — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test Criteria' };
