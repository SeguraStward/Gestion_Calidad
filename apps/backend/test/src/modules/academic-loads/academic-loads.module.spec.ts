...
describe('AcademicLoadsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<AcademicLoadsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test AcademicLoads',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [AcademicLoadsModule],
    })
      .overrideProvider(AcademicLoadsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<AcademicLoadsRepository>(AcademicLoadsRepository) as any;
  });
  ...
  describe('Flujos HTTP - AcademicLoads', () => {
    it('POST /academic-loads — Crea registro — responde 201', async () => {
      const createDto = { 
        name: 'Test AcademicLoads', 
        courseId: 'asd', 
        academicCycleId: 'asd', 
