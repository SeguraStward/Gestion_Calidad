...
import { CommMembersModule } from '@modules/comm-members/comm-members.module';
import { CommMembersRepository } from '@modules/comm-members/comm-members.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('CommMembersModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<CommMembersRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test CommMembers',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [CommMembersModule],
    })
      .overrideProvider(CommMembersRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<CommMembersRepository>(CommMembersRepository) as any;
  });
  ...
  describe('Flujos HTTP - CommMembers', () => {
    it('POST /comm-members — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test CommMembers' };
