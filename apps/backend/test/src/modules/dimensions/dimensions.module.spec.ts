...
import { DimensionsRepository } from '@modules/dimensions/dimensions.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('DimensionsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<DimensionsRepository>;
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .overrideProvider(DimensionsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<DimensionsRepository>(DimensionsRepository) as any;
  });
  ...
  describe('Flujos HTTP - Dimensions', () => {
    it('POST /dimensions — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test Dimensions' };
