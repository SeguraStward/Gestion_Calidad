...
import { AcademicBackgroundsRepository } from '@modules/academic-backgrounds/academic-backgrounds.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('AcademicBackgroundsModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<AcademicBackgroundsRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test AcademicBackgrounds',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [AcademicBackgroundsModule],
    })
      .overrideProvider(AcademicBackgroundsRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<AcademicBackgroundsRepository>(AcademicBackgroundsRepository) as any;
  });
  ...
  describe('Flujos HTTP - AcademicBackgrounds', () => {
    it('POST /academic-backgrounds — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test AcademicBackgrounds' };
    ...
    it('GET /academic-backgrounds — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/academic-backgrounds?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /academic-backgrounds/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/academic-backgrounds/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /academic-backgrounds/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/academic-backgrounds/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /academic-backgrounds/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/academic-backgrounds/test-id-1');
