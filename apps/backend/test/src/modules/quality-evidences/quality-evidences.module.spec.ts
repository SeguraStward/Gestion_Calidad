...
import { QualityEvidencesModule } from '@modules/quality-evidences/quality-evidences.module';
import { QualityEvidencesRepository } from '@modules/quality-evidences/quality-evidences.repository';
import { DtoValidator } from '@core/common/dto-validator';

describe('QualityEvidencesModule (Modular)', () => {
  let app: INestApplication;
  let repository: jest.Mocked<QualityEvidencesRepository>;

  const mockEntity = {
    id: 'test-id-1',
    name: 'Test QualityEvidences',
    status: 'ACTIVE',
  };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      imports: [QualityEvidencesModule],
    })
      .overrideProvider(QualityEvidencesRepository).useValue(mockRepository)
    ...
    repository = moduleFixture.get<QualityEvidencesRepository>(QualityEvidencesRepository) as any;
  });
  ...
  describe('Flujos HTTP - QualityEvidences', () => {
    it('POST /quality-evidences — Crea registro — responde 201', async () => {
      const createDto = { name: 'Test QualityEvidences' };
    ...
    it('GET /quality-evidences — Lista registros — responde 200', async () => {
      repository.findAll.mockResolvedValue({ data: [mockEntity as any], meta: { total: 1 } } as any);

      const response = await request(app.getHttpServer())
        .get('/quality-evidences?page=1&limit=10')
        .expect(200);

      expect(response.body.data).toHaveLength(1);
    });

    it('GET /quality-evidences/:id — Obtiene un registro — responde 200', async () => {
      repository.findById.mockResolvedValue(mockEntity as any);

      const response = await request(app.getHttpServer())
        .get('/quality-evidences/test-id-1')
        .expect(200);

      expect(response.body.id).toBe('test-id-1');
    });

    it('PUT /quality-evidences/:id — Actualiza registro — responde 204 o 200', async () => {
      const updateDto = { name: 'Updated' };
      repository.update.mockResolvedValue({ ...mockEntity, ...updateDto } as any);

      const res = await request(app.getHttpServer())
        .put('/quality-evidences/test-id-1')
        .send(updateDto);

      if (res.status !== 200 && res.status !== 204) throw new Error('Expected status 200 or 204, got ' + res.status);
    });

    it('DELETE /quality-evidences/:id — Elimina registro — responde 200 o 204', async () => {
      repository.deleteById.mockResolvedValue(true);
      repository.delete.mockResolvedValue(true as any);
      repository.findById.mockResolvedValue(mockEntity as any);

      const res = await request(app.getHttpServer())
        .delete('/quality-evidences/test-id-1');
