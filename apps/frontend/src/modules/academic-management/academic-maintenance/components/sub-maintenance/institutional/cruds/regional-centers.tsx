...
const regionalCenterConfig: CrudConfig<RegionalCenterItem, CreateRegionalCenterInput, UpdateRegionalCenterInputType> = {
  ...
  processItem: (item: RegionalCenterWithRelations): RegionalCenterItem => ({
    ...item,
    id: item.id,
    campuses: (item.campuses || []).map((campus: PrismaCampus): ProcessedCampus => ({ // Use PrismaCampus here
      id: campus.id,
  ...
  renderForm: ({ control, errors, isProcessing, isUpdate, handleCancel, handleSubmitForm }: {
    ...
    >
      <div className="space-y-1">
        ...
        <Controller
          ...
          name="code"
          render={({ field }) => (
            <Input id="code" placeholder="Ej: SR-PACIFICO" {...field} />
          )}
        />
      ...
      <div className="space-y-1">
        ...
        <Controller
          ...
          name="name"
          render={({ field }) => (
            <Input id="name" placeholder="Ej: Sede Regional Pacífico Central" {...field} />
          )}
        />
  ...
  renderItem: (item: RegionalCenterItem, { onEdit, onDelete, isProcessing, isEditing }: {
    onEdit: () => void;
    onDelete: () => void;
    isProcessing: boolean;
    isEditing: boolean;
          ...
          {item.campuses?.length > 0 ? (
            <ul className="list-disc pl-5 text-sm">
              {item.campuses.map((campus: ProcessedCampus) => (
                <li key={campus.id}>{campus.name} ({campus.code})</li>
              ))}
            </ul>
          ) : (
...
export default function RegionalCenterPage() {
  return <CrudModuleBase<RegionalCenterItem, CreateRegionalCenterInput, UpdateRegionalCenterInputType> {...regionalCenterConfig} />
}