          ...
          sections={() => [
            {
              ...
              fields: [
                ...
                },
                {
                  ...
                  label: 'Escuela',
                  required: true,
                  options: schools.map((s: any) => ({ id: s.id, name: s.name || '', status: s.status })),
  ...
  const crudConfig = useMemo(
    () => ({
      ...
      processFormValues: (values: any) => {
        ...
        return { code, name, schoolId, status }
      },
      preDeleteCheck: (item: CareerItem) => {
        if (item.courses?.length) return 'No se puede eliminar una carrera con cursos asociados.'
        if (item.projects?.length) return 'No se puede eliminar una carrera con proyectos asociados.'
        return null
      }
    }),
