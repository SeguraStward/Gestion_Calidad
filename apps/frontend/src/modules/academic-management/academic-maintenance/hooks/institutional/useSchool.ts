...
const baseHooks = createGenericHooks<
  SchoolWithRelations,
  CreateSchoolInput,
...
export const useListSchools = (filters?: Omit<SchoolFilters, 'page' | 'limit'>) => {
  return useQuery<SchoolWithRelations[], Error>({
    ...
    queryFn: async () => {
      ...
      // Ensure response is not null and is an object before checking hasOwnProperty
      if (response && typeof response === 'object' && response.hasOwnProperty('items') && Array.isArray((response as any).items)) {
      ...
      console.warn('❌ [useListSchools] Could not extract school list from response. Unexpected structure or empty response:', response);
