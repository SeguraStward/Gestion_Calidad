...
export const formHelpers = {
  ...
  createDefaultValues: <T extends Record<string, any>>(fields: Partial<T>, includeRelations: boolean = true): T => {
...
export const uiHelpers = {
  ...
  getEditingClasses: (isEditing: boolean) => ({
    card: `transition-all duration-200 hover:shadow-md ${isEditing ? 'ring-2 ring-blue-500 shadow-lg' : ''} overflow-hidden`,
...
export const errorHelpers = {
  ...
  isDuplicateError: (error: any) => {
    ...
    return message.includes('duplicate') || message.includes('unique') || message.includes('ya existe')
  }
}
